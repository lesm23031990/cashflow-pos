const { consultar, ejecutar, primero } = require('./connection');
...
// Carga datos demo SOLO si la DB está vacía (sin facturas)
function sembrarDemo() {
  const facturaCount = consultar('SELECT COUNT(*) AS count FROM facturas');
  if (facturaCount[0].count > 0) {
    console.log('seed-demo: ya existen facturas, no se recarga.');
    return;
  }

  // Productos
  const prodCount = consultar('SELECT COUNT(*) AS count FROM productos');
  if (prodCount[0].count === 0) {
    ...
    console.log(`seed-demo: ${PRODUCTOS.length} productos cargados`);
  }

  // Clientes
  const cliCount = consultar('SELECT COUNT(*) AS count FROM clientes');
  if (cliCount[0].count === 0) {
    ...
    console.log(`seed-demo: ${CLIENTES.length} clientes cargados`);
  }
  ...
  // Facturas en las últimas 2 horas
  const numFacturas = rand(15, 25);
  let minutos = 0;
  for (let i = 0; i < numFacturas; i++) {
    minutos += rand(3, 10);
    if (minutos > 120) break;

    const fecha = fechaHace(minutos);
    const numItems = rand(1, 8);
    const items = [];
    for (let j = 0; j < numItems; j++) {
      items.push({ prod: pick(productos), cant: rand(1, 4) });
    }
    ...
    ejecutar(
      ...
      [cliente.id, fecha, 'COP', tasas.usd, tasas.ves, subtotal, descuento, total, status, metodo, status === 'en espera' ? 'En espera' : '']
    );
  ...
  console.log(`seed-demo: ${numFacturas} facturas demo generadas en las últimas 2 horas`);
}
